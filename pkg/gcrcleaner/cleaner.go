...
func (c *Cleaner) Clean(ctx context.Context, repo string, since time.Time, keep int64, tagFilter TagFilter, tagFilterOut, dryRun bool) ([]string, error) {
	...
	for _, m := range manifests {
		...
		if !c.shouldDelete(m, since, tagFilter, tagFilterOut) {
...
func (c *Cleaner) shouldDelete(m *manifest, since time.Time, tagFilter TagFilter, tagFilterOut bool) bool {
